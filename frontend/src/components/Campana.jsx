      ...
      {abierto && (
        <div className="absolute right-0 top-10 w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            {noLeidas > 0 && (
              <button onClick={marcarLeidas} className="text-xs text-indigo-600 hover:text-indigo-800">
                Marcar todas leídas
              </button>
            )}
          </div>
          ...
          <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
            {notifs.length === 0 ? (
              ...
              return (
                <div key={n.id} className={`px-4 py-3 group ${n.leida ? '' : 'bg-indigo-50/40'}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-start gap-2 min-w-0">
                      <span className={`mt-0.5 text-xs px-1.5 py-0.5 rounded font-medium flex-shrink-0 ${cfg.color}`}>
                        {cfg.label}
                      </span>
                      {!n.leida && <span className="w-2 h-2 bg-indigo-500 rounded-full flex-shrink-0 mt-1" />}
                    </div>
